...
int main(int argc, char **argv) {
  ...
  // convert step_ms to seconds then multiple by sample rate to get number of
  // samples (default will give us (0.001 * 3000) * 16000 = 48000 samples)
  const int n_samples_step = (1e-3 * params.step_ms) * WHISPER_SAMPLE_RATE;

  const int n_samples_len = (1e-3 * params.length_ms) * WHISPER_SAMPLE_RATE;
  const int n_samples_keep = (1e-3 * params.keep_ms) * WHISPER_SAMPLE_RATE;

  const int n_samples_30s = (30.0) * WHISPER_SAMPLE_RATE;

  // if the number of samples in the step is less than or equal to 0, use voice
  // activated detection
  const bool use_vad = n_samples_step <= 0;

  // number of steps to print new line
  // if we are using vad, we want to print a new line every step
  //
  // if we are not using vad, we want to print a line one step less than the
  // number of steps that fit in the audio length so in the default case this
  // would every 2 steps
  const int n_new_line =
      !use_vad ? std::max(1, params.length_ms / params.step_ms - 1) : 1;

  // if we are using vad, we want to keep the context between steps
  params.no_timestamps = !use_vad;

  // if we are using vad we will not keep context between steps
  // otherwise use the provided option setting, defaults to false
  params.no_context |= use_vad;

  // maximum tokens in a single audio chunk
  params.max_tokens = 0;
  ...
  // if the language param is not recognized throw an error
  ...
  // whisper init, use model param for model file path
  ...
  while (is_running) {
    ...
    // in this case handles Ctrl + C for quitting
    // see implementation in sdl.cpp for details
    is_running = sdl_poll_events();

    // if user has killed the program with Ctrl + C
    ...
    // VAD mode
      ...
      // take a timestamp of the current time
      const auto t_now = std::chrono::high_resolution_clock::now();

      // calculate the time in milliseconds since the last iteration
      const auto t_diff =
          std::chrono::duration_cast<std::chrono::milliseconds>(t_now - t_last)
              .count();

      // if it has been less than 2 seconds since the last iteration
      ...
      // get the next 2 seconds of audio, and store it in pcmf32_new
      audio.get(2000, pcmf32_new);

      // pass this audio to the voice activated detector
      // if there is audio found that is above the threshold
      // we read the whole length (10s by default) into pcmf32
      if (::vad_simple(pcmf32_new, WHISPER_SAMPLE_RATE, 1000, params.vad_thold,
                       params.freq_thold, false)) {
        audio.get(params.length_ms, pcmf32);
      }
      // if ther is no audio found above the threshold sleep another 100 ms
      else {
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
        continue;
      }
    ...
    // now that we have audio data stored to pcmf32 we can process it
    // with the model
    {
